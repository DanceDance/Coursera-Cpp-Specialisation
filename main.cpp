...
public:
  ...
  AirportCounter(TIterator begin, TIterator end) : AirportCounter() {
    for (auto it = begin; it != end; it++) {
      db_[static_cast<size_t>(*it)]++;
    }
  }

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const {
    return db_[static_cast<size_t>(airport)];
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
    db_[static_cast<size_t>(airport)]++;
  }
  ...
  void EraseAll(TAirport airport) {
    db_[static_cast<size_t>(airport)] = 0;
  }
  ...
  Items GetItems() const {
    Items items;
    for (size_t i = 0; i < db_.size(); i++) {
      items[i] = Item(static_cast<TAirport>(i), db_[i]);
    }
    return items;
  }

private:
  array<size_t, static_cast<size_t>(TAirport::Last_)> db_;
};
